    ...
    func search(text: String) async {
        ...
        do {
            for  group in self.groups {
                ...
                //let group: SearchGroup = SearchGroup(field: field, alias: field, features: features.map(SearchItem.init))
                
               // group.features.append(features.map(SearchItem.init))
                //self.groups.append(group)
                //self.group.features.append(features.map(SearchItem.init))
                //self.features = features.map(SearchItem.init)
            }
