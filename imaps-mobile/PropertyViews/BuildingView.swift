    ...
    var body: some View {
        Group {
            ...
            Grid() {
                GridRow {
                    Text("Heated Area")
                    Text( ((attributes["HEATEDAREA"] as? Double ?? 0)
                ...
                GridRow {
                    ...
                    Text( String((( unwrap(any: attributes["YEAR_BUILT"]) as? Int16 ?? 0))))


                   // Text(unwrap(any: feature?.attributes["YEAR_BUILT"]) as? String ?? "")
//                    Text(feature?.attributes["YEAR_BUILT"] as? String ?? "")
                ...
                GridRow {
                    Text("Design/Style")
                    Text(attributes["DESIGN_STYLE_DECODE"] as? String ?? "")
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(.all, 1)
                ...
                GridRow {
                    Text("Use Type")
                    Text(attributes["TYPE_USE_DECODE"] as? String ?? "")
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(.all, 1)
                ...
                GridRow {
                    ...
                    Text( String((( unwrap(any: attributes["TOTSTRUCTS"]) as? Int16 ?? 0))))
                ...
                GridRow {
                    Text("Total Units")
                    Text( String((( unwrap(any: attributes["TOTUNITS"]) as? Int16 ?? 0))))
