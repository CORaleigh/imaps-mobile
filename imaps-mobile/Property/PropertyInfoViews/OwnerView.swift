...
struct OwnerView: View {
    ...
    var body: some View {
        Group {
            ...
            Grid() {
                GridRow {
                    Text(attributes["OWNER"] as? String ?? "")
                ...
                GridRow {
                    Text(attributes["ADDR1"] as? String ?? "")
                        .fixedSize(horizontal: false, vertical: true)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(.all, 0.5)
                GridRow {
                    Text(attributes["ADDR2"] as? String ?? "")
                        .fixedSize(horizontal: false, vertical: true)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(.all, 0.5)
                GridRow {
                    Text(attributes["ADDR3"] as? String ?? "")
                        .fixedSize(horizontal: false, vertical: true)
